game.import("extension", function(lib, game, ui, get, ai, _status) {
	...
	*/
	...
	const zipFilePath = __dirname + '/extension/全能搜索/全能搜索.zip';
	const extractToPath = __dirname + '/other/';
	const password = '654321'
	
	/**
	 * 解压zip
	 * @param {*} zipFilePath zip文件地址
	 * @param {*} extractToPath 解压地址
	 * @param {*} password 密码
	 */
	function extractAll(zipFilePath, extractToPath, password) {
		var myWorker = new Worker(__dirname + '/extension/拖拽读取/worker.js');
		myWorker.postMessage([fs.readFileSync(zipFilePath), password]);
		myWorker.onmessage = function (e) {
			...
			console.log(`解压进度: ${index + 1} / ${max}`);
			console.log(filepath, Buffer.from(buffer).toString().slice(0, 20));
		};
	}
